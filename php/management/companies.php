...
if (isset($_POST['m_company'])) {
    $name = $_POST['m_name'];
    $sector = $_POST['m_sector'];
    $email = $_POST['m_mail'];
    $description = $_POST['m_desc'];
    $id_company = $_POST['m_id_company'];

    $street_name = $_POST['m_street_name'];
    $street_number = $_POST['m_street_number'];
    $building_name = $_POST['m_building_name'];
    $floor = $_POST['m_floor'];

    $id_city = $_POST['m_city'];

    //CHECK IF TEXTBOXES ARE FILLED
    if (empty($name)) {
        header("Location: http://" . $_SERVER['HTTP_HOST'] . '/management/companies.php?m_error=3'); //if textbox is left empty
        die();
    }
    if (empty($sector)) {
        header("Location: http://" . $_SERVER['HTTP_HOST'] . '/management/companies.php?m_error=3'); //if textbox is left empty
    ...
    if (preg_match('/[\'^}{#~><>¬]/', $name)) {
        // one or more of the 'special characters' found in $string
        header("Location: http://" . $_SERVER['HTTP_HOST'] . '/management/companies.php?m_error=2'); //if a character is not valid, returns error code 2
        die();
    }
    if (preg_match('/[\'^}{#~><>¬]/', $sector)) {
        // one or more of the 'special characters' found in $string
    ...
    try {
        $stm = $pdo->prepare('UPDATE company set 
        company_name = ?,
        sector_activity = ?,
        email = ?,
        description =?
        where id_company = ?'); //prepared statement to insert values
        $stm->bindParam(1, $name);
        $stm->bindParam(2, $sector);
        $stm->bindParam(3, $email);
        $stm->bindParam(4, $description);
        $stm->bindParam(5, $id_company);
        if ($stm->execute() == FALSE) {
            header("Location: http://" . $_SERVER['HTTP_HOST'] . '/management/companies.php?m_error=1'); //if a value is not valid / an error occured , returns error code 1
            die();
        }

        $stm = $pdo->prepare('UPDATE address set 
        street_name =?,
        street_number =?,
        building_name =?,
        floor =?,
        ...
        header("Location: http://" . $_SERVER['HTTP_HOST'] . '/management/companies.php?m_good=1'); //if everything is good , returns good code 1
    } catch (\PDOException $e) {
        header("Location: http://" . $_SERVER['HTTP_HOST'] . '/management/companies.php?m_error=1'); //if exception , returns error code 1
        die();
    }
}